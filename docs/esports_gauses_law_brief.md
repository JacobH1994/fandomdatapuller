...
**A structural Classic-vs-Second-Coming difference found while working the CS/Dota 2 lifecycle briefs (`docs/counter_strike_lifecycle_brief.md` §15, 2026-09-09), not this brief's own line of investigation but directly relevant to the era framework above**: distinct monthly Twitch streamer counts (`Streamers`, Kaggle) show a large, consistent gap between eras, not a single-title quirk — Fortnite reaches 7.1-11.2M distinct monthly streamers (5-10x any Classic title), VALORANT and Apex Legends both exceed every Classic title except League of Legends. Dota 2 sits at the opposite extreme, the smallest of six titles compared despite total viewership comparable to some Second Coming titles. Stated as a hypothesis there, not proven: newer titles may structurally support a far larger long-tail creator population than older, broadcast-centric titles do — a possible mechanism worth keeping in mind if H1-H3's differentiation-vector work ever turns to creator-side (not just viewer-side) evidence.
