...
- **`config/channels.yaml` being empty for CS is not a blocker here, confirmed 2026-09-09.** This brief's esports-specific signal is `broadcast_tier` (title-text + tournament-date-window matching, `etl/classify_broadcast_tier.py`), not `is_official_broadcast` — the latter is the field `channels.yaml` actually gates, and it isn't used anywhere in this design. Worth stating precisely, since the two are easy to conflate: `is_official_broadcast` is baked into each stream's raw snapshot at *capture time* (`collectors/twitch_poll.py`, deliberate — channels change season to season, so a later `channels.yaml` must not retroactively relabel an earlier era), so filling `channels.yaml` in only affects polls from that point forward and can never backfill it for anything already collected. `broadcast_tier`, by contrast, doesn't look at channel identity at all and can be (re)computed over the full existing history at any time — which is exactly why it, not `is_official_broadcast`, is what this brief relies on. Re-running the eventual notebook after `channels.yaml` is populated is easy either way (every notebook here just re-reads `research.db`), but won't change this brief's numbers, since they never depended on it.
