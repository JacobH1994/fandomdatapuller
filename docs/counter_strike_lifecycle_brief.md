...
- **`config/channels.yaml` being empty for CS: real but partial impact, corrected 2026-09-09 after actually running the classifier** (an earlier version of this note claimed no impact at all — checking the live result changed that). Two genuinely different fields, easy to conflate: `is_official_broadcast` (what `channels.yaml` gates, baked in at *capture time* — `collectors/twitch_poll.py`, deliberately, since channels change season to season and a later `channels.yaml` must not retroactively relabel an earlier era) vs. `broadcast_tier` (computed after the fact, retroactively re-runnable over full history at any time). This brief's esports-specific signal is `broadcast_tier`, not `is_official_broadcast` directly — but `broadcast_tier`'s own `primary_official` value *is* just `is_official_broadcast=1` read back, so it inherits the same capture-time dependency one level removed: confirmed 0 `primary_official` rows platform-wide with `channels.yaml` empty, not fixable retroactively once channels are curated. The `detected_costream` tier (title/date/alias text match) doesn't have this problem and catches most genuine official broadcasts anyway — but not ones with a generic, non-tournament-referencing title, which fall through to `general` uncaught (§4's real numbers: 919 `detected_costream` vs. 31,785 `general` for CS). Net: populating `channels.yaml` going forward would improve both the confidence (`verified` vs. `proxy_estimate`) and the completeness (catching generically-titled official streams) of this metric *from that point on* — worth prioritizing before treating this brief's esports-specific-attention numbers as final, not merely a nice-to-have.
